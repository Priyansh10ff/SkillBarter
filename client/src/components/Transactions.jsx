...
const Transactions = () => {
  ...
  const handleComplete = async (id) => {
    try {
      ...
      fetchTransactions();
      refreshUser();
    } catch (error) { toast.error("Error completing transaction"); }
  };
  ...
  const handleSchedule = async (id, action) => {
    ...
    try {
      ...
      await axios.put(`http://localhost:5000/api/transactions/${id}/schedule`, 
        { date: scheduleDate, action }, 
        { headers: { Authorization: `Bearer ${token}` } }
      );
      toast.success(action === 'PROPOSE' ? "Time Proposed!" : "Time Accepted!");
      fetchTransactions();
    } catch (error) { toast.error("Scheduling failed"); }
  };
      ...
      <div className="container mx-auto max-w-4xl">
        ...
        <div className="space-y-4">
          {filteredTransactions.map((tx, index) => {
            ...
            const needsScheduling = tx.appointmentStatus === 'UNSCHEDULED';
